...
def clearConsole():
    if osName == "nt":
        ...
        osSystem("clear")
'''
def RecoverAccount(attemptPrompt):
    clearConsole()
    print("----------------------------------------")
    print("----------------------------------------")
    ...
    print("----------------------------------------")
    print("----------------------------------------")
    if attemptPrompt != None:
        print(attemptPrompt)
    print("1. Recover Account?")
    print("2. Back?")
    while True:
        option = input("Choose one from the above:")
        if option == 1 or option == "1":
            while True:
                keyInput = input(str("Enter recovery key:"))
                if keyInput == Configuration['loginCredential']['recoveryKey']:
                    newUsername = input(str("new Username:"))
                    newPassword = input(str("new Password:"))
                    confirmPassword = input(str("Confirm new Password:"))

                    if newPassword != confirmPassword:
                        print("Confirm Password mismatch, Please try again!")
                        continue

                    if newPassword == confirmPassword:
                        Configuration["loginCredential"]["username"] = newUsername
                        Configuration["loginCredential"]["password"] = confirmPassword
                        break
                if keyInput != Configuration['loginCredential']['recoveryKey']:
                    clearConsole()
                    ...
                    print("1. Try again")
                    print("2. Back")
                    option = input(str("Choose one option above:"))
                    if option == "1" or option == 1:
                        continue
